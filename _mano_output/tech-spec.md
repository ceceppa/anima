...
**Target freed → cancel affected motion safely** is the one case with no existing mechanism, and the one this phase actually implements: it is the same failure class the Phase 10 review flagged as a leak (a discarded-but-uncancelled playback still sitting in `active_playbacks`, being advanced against a target that's gone). `AnimaPlayback._init()` connects `target.tree_exiting` to `cancel()` whenever `target != null` — event-driven, not a per-frame `is_instance_valid()` poll, and it fires for both runtime-tracked and manually-constructed playbacks alike, since it lives in `_init()` rather than in `AnimaRuntime._track()`.
...
- `AnimaPlayback._init()` connects `target.tree_exiting` to `cancel()` whenever a target is supplied — the fix for the Phase 10 review's discarded-but-uncancelled-playback leak, and the only new mechanism `Lifecycle-safe playback policies` requires; the other five recommended defaults already hold via existing Godot/Anima behaviour and needed no code change, only documenting.
