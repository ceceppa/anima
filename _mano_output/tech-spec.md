...
When active and `reduced_motion_speed >= 0.0`, `_advance()` uses that value as the effective speed outright, replacing the `speed_scale × direction_speed` product for that frame (motion-level `.speed` still applies underneath it, same as any other effective-speed consumer) — superseding whatever speed was otherwise chosen, per the decision. When active but no `reduced_motion_speed` is set, nothing changes; this still implements only one of PRD5's four described reduced-motion behaviours ("applying a dedicated reduced-motion speed") — completing immediately, swapping to a simpler motion, and removing stagger stay deferred with the fuller per-motion policy (`FULL`/`SHORTEN`/`SIMPLIFY`/`COMPLETE_IMMEDIATELY`), still backlogged.
...
| | `reduced_motion_speed` | `float`, default `-1.0` (no override) | When reduced motion is active for this playback (see §Speed, direction, and reduced motion's three-way resolution) and this is `>= 0.0`, replaces `speed_scale × direction_speed` for this playback | see §Speed, direction, and reduced motion |
