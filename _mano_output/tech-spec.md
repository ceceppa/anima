...
| Entity | Fields | Notes |
|---|---|---|
| `AnimaMotion` | display name, enabled, delay, speed, tags, metadata, `on_started_callback`, `on_completed_callback` | Base Resource for all leaf and composite motions. The two callback fields are optional `Callable`s invoked by `AnimaPlayback`, not signals — see §Target-bound authoring contract. |
