...
uniform float range: hint_range(0, 1) = 1.0;
uniform float uv_multiplier = 20.0;
uniform vec4 initial_color: hint_color = vec4(0.0, 0.0, 0.0, 1.0);
uniform vec4 final_color: hint_color = vec4(1.0, 0.0, 0.0, 1.0);

void fragment() {
	vec4 color = texture(TEXTURE, UV);
	
	float ratio = step(UV.x * uv_multiplier, range);
