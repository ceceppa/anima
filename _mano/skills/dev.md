...
- **Respect order.** If an earlier story has a Status other than `done`, stop and tell the user which story would be skipped. Do not bypass without explicit confirmation.
...
- **Spec-owned defaults are not story choices.** If an AC needs a starting state, capacity, radius/range, count, duration, threshold, spawn amount, or other behaviour-driving default that the cited spec does not own and define, apply `AGENTS.md` step 6.2: stop and route to `mano spec`. Do not supply a temporary literal or a “story-owned default.”
- **Player choices are not story-owned interaction design.** If an AC gives the player a choice among simultaneously available alternatives but its cited UX flow does not define selection, active-state feedback, and locked/unavailable/cancel behaviour, apply `AGENTS.md` step 6.3: stop and route to `mano ux`. Do not invent a hotkey, picker, cycling scheme, default active item, or HUD treatment.
- **A user may expand scope, but Dev cannot hide it.** Apply `AGENTS.md` step 6.4 before code: if a story or requested behaviour is a distinct phase outcome or conflicts with `Not this phase`, stop. Ask the human to defer it or amend the phase brief, then rerun `mano stories`; “do it anyway” alone is not authority to leave the phase contract stale. This applies in YOLO and auto mode too.
...
- **Final story must close the phase contract.** Before implementing the last pending row, apply `AGENTS.md` step 6.1. Every Phase goal element and Exit Criterion needs an AC for the same user/caller path; an alternative entry point is not equivalent. A missing path routes to `mano stories`, or to `mano spec` first when the shared/public contract is absent. This gate remains active in YOLO and auto mode.
...
- **Green tests cannot prove the opposite AC.** Before setting status to `done`, apply `AGENTS.md` step 10.1 to every acceptance criterion. If a test, comment, fixture, or observed result expects the inverse outcome, leave the story pending and report the contradiction; never invert the AC to match stale code or an older artifact.
...
- **Never empty the project for a scaffold.** Greenfield project generators must use the exact `scaffold.js run ... {target}` command from the tech spec. Never move, delete, rename, or temporarily hide `_mano`, `_mano_output`, Git metadata, agent instructions, or any existing file; never hand-merge staged output. A missing command routes to `mano spec`; a script failure or collision stops the story. This remains a hard stop in `yolo` and auto mode.
- **One-line done (default mode).** After implementing, confirm the writer and post-write state checks from `AGENTS.md` step 11. Then the entire chat response is `Story [N] done — status updated in stories/README.md`. No recap, no checklist, no AC restatement. The only permitted additions are a non-acceptance deviation that did not weaken verification or a project-relevant decision worth preserving. An unmet AC always leaves the story pending. Nothing else.
