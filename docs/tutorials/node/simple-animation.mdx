...
Also, we can use either [begin](/docs/anima/#begin) or [begin_single_shot](/docs/anima/#begin_single_shot) without difference in how the animation is created/played. Is completely up to you to decide [which one suits your need](/docs/tutorials/basics/begin-vs-begin_single_shot).

### The Panel

Because we're animating a single node we're going to use the [Anima.Node](/docs/anima/anima-node) utility class and its [anima_scale_y](/docs/anima-declaration/#anima_scale_y) method:
...
Once again we used the [Anima.Node](/docs/anima/anima-node) utility class because we're animating a single node. While the [anima_animation](/docs/anima-declaration/#anima_animation) let us specify the name of the animation to use and its duration.
...
We could define the animation as parallel, using the [with](/docs/anima-node/#with-parallel-animations) syntax, but because the text animation depends by its length we would need to manually calculate the delay to apply.
...
:::note
If we have a look at the [anima_delay](/docs/anima-declaration/#anima_delay) documentation, we can see that we can also pass a negative number to anticipate the start of a sequential or parallel animation.
:::